...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   // log << "inside reply_put " << endl; // debugging purposes
   // recieve payload and store it
   char * buffer = new char [header.nbytes+1];
   recv_packet(client_sock, buffer, header.nbytes);

   // log << "received header and buffer " << endl;

   // check to make sure payload exists
   if (buffer == NULL) {
      log << "put: failed: " << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      return;
   ...
   // return acknowledgement of file transfer
   header.command = cix_command::ACK;
   memset (header.filename, 0, FILENAME_SIZE);
   log << "sending header " << header << endl;
   send_packet (client_sock, &header, sizeof header);
   log << "sent " << sizeof header << " bytes" << endl;
...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (is) {
      ...
      log << "sending header " << header << endl;
      send_packet(client_sock, &header, sizeof header);
      send_packet(client_sock, &bufferstring, header.nbytes);
      log << "sent " << sizeof header << " bytes" << endl;
      log << "sent " << sizeof bufferstring << endl;
